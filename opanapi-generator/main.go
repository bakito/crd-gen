...
func generateTypesCode(structMap map[string]*StructDef, version, crdKind, plural, crdGroup, crdVersion, crdList string) string {
...
func generateGroupVersionInfoCode(group, version string, kinds []string, lists []string) string {
	var sb strings.Builder

	writef(&sb, "// Code generated by %s. DO NOT EDIT.\n\n", myName)
	writef(&sb, "package %s\n\n", version)

	write(&sb, "// +kubebuilder:object:generate=true\n\n")

	write(&sb, "import (\n")
	write(&sb, "\t\"k8s.io/apimachinery/pkg/runtime/schema\"\n")
	write(&sb, "\t\"sigs.k8s.io/controller-runtime/pkg/scheme\"\n")
	write(&sb, ")\n\n")

	write(&sb, "var (\n")
	write(&sb, "\t// GroupVersion is group version used to register these objects.\n")
	write(&sb,
		fmt.Sprintf("\tGroupVersion = schema.GroupVersion{Group: %q, Version: %q}\n\n", group, version),
	)

	write(&sb, "\t// SchemeBuilder is used to add go types to the GroupVersionKind scheme.\n")
	write(&sb, "\tSchemeBuilder = &scheme.Builder{GroupVersion: GroupVersion}\n\n")

	write(&sb, "\t		// AddToScheme adds the types in this group-version to the given scheme.\n")
	write(&sb, "\tAddToScheme = SchemeBuilder.AddToScheme\n")
	write(&sb, ")\n\n")

	write(&sb, "func init() {\n")
	write(&sb, "\t// Register types with the scheme\n")
	sort.Strings(kinds)
	for i, kind := range kinds {
		writef(&sb, "\tSchemeBuilder.Register(&%s{}, &%s{})\n", kind, lists[i])
	}
	write(&sb, "}\n\n")
...
func main() {
	...
	for i, crd := range crds {
		...
		kinds = append(kinds, crdKind)
		lists = append(lists, crdList)
		...
		// Generate types code
		typesCode := generateTypesCode(structMap, version, crdKind, crdPlural, crdGroup, version, crdList)
	...
	gvi := generateGroupVersionInfoCode(crdGroup, version, kinds, lists)
